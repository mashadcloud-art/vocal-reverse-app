...
app.post('/api/download-url', async (req, res) => {
  ...
  try {
    ...
    const metadata = await new Promise((resolve, reject) => {
      exec(`yt-dlp --js-runtimes node --username oauth2 --password "" --get-title --get-thumbnail --get-id --no-playlist "${url}"`, (error, stdout) => {
    ...
    await new Promise((resolve, reject) => {
      ...
      exec(`yt-dlp --js-runtimes node --username oauth2 --password "" --no-playlist -f "ba" -x --audio-format ${formatFlag} ${qualityFlag} -o "${outputPath}" "${url}"`, (error, stdout, stderr) => {
        if (error) {
